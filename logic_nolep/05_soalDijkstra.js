...
function dijkstra(graph, start, end) {
  ...
  while (!pq.isEmpty()) {
    ...
    for (let nextCity in graph[currentCity]) {
      const distance = distances[currentCity] + graph[currentCity][nextCity];
      if (distance < distances[nextCity]) {
        distances[nextCity] = distance;
        previous[nextCity] = currentCity;
        pq.enqueue(nextCity, distance);
      }
    }
  }
