...
function dijkstra(graph, start, end) {
    ...
    // initialize the initial distance and put all nodes into the priority queue.
    ...
    while (!pq.isEmpty()) {
        // take the node with the lowest weight from the queue.
        const currentVertex = pq.dequeue();

        // check whether the target node has been reached.
        ...
        // check each neighbor nodes
        for (const neighbor in graph[currentVertex]) {
            if (visited.has(neighbor)) {
                continue;
            }

            // calculate the distance from the current node to the neighbor nodes
