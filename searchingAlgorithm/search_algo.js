...
const searchRotatedArray = (nums, target) => {
  ...
  while (start <= end) {
    ...
    // Langsung kembalikan indeks jika target ditemukan
    if (nums[mid] === target) {
      return mid;
    }

    // Tentukan sisi yang terurut dan apakah target berada di sisi itu
    if (nums[start] <= nums[mid]) {
      // Sisi kiri terurut
      if (target >= nums[start] && target < nums[mid]) {
        end = mid - 1; // Cari di kiri
      } else {
        start = mid + 1; // Cari di kanan
      }
    } else {
      // Sisi kanan terurut
  ...
  return -1; // Jika loop selesai tanpa menemukan target, kembalikan -1
};
