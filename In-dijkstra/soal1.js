...
function dijkstra(graph, start, end) {
  ...
  console.log("Jarak terdekat dari Jakarta ke Surabaya:");

  while (!pq.isEmpty()) {
    const currentVertex = pq.dequeue();

    if (currentVertex === end) {
      console.log(`\nTitik tujuan ${end} telah didapat.`);
      break;
    }
    ...
    for (let neighbor in graph[currentVertex]) {
      ...
      console.log(
        `  Tetangga ${neighbor}: jarak saat ini = ${distances[neighbor]}, jarak baru = ${distance}`
      );
